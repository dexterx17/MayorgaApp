...
public class TipoClienteDB {
    DBHelper db;
    private static final String TABLA = "TIPOCLIENTE";
    ...
    private static final String DB_CREATE = "create table "
            +TABLA
            +" ( "
            +C_CODIGO + " integer not null primary key, "
            +C_DESCRIPCION + " varchar(250), "
            +C_ACTUALIZACION + " datetime );";

    /**
     * Constructor de la clase TipoClienteDB
     *  obtiene una instancia de la conexión a la BDD
     * @param context
     */
    public TipoClienteDB(Context context) {
     ...
     * @return ArrayList
     */
    public ArrayList<TipoCliente> getList() {
        Cursor cursor= db.select("select * from TIPOCLIENTE",null);
        ArrayList<TipoCliente> tipos =  TipoCliente.fromCursor(cursor);
        cursor.close();
        return tipos;
    }

    /**
     * Inserta un TipoCliente en la BDD
     * @param tipo
     * @return
     */
    public long insert(TipoCliente tipo){
        ContentValues valores = new ContentValues();
        valores.put(C_CODIGO, tipo.CODIGO);
        valores.put(C_DESCRIPCION, (String) tipo.DESCRIPCION.toString().trim());
        valores.put(C_ACTUALIZACION, String.valueOf(tipo.ACTUALIZACION));
        return db.insert(TABLA,valores);
    }

    /**
     * Edita un TipoCliente de la BDD
     * @param tipo
     * @return boolean true/false
     */
    public boolean update(TipoCliente tipo){
        ContentValues valores = new ContentValues();
        valores.put(C_DESCRIPCION, (String) tipo.DESCRIPCION.toString().trim());
        valores.put(C_ACTUALIZACION, String.valueOf(tipo.ACTUALIZACION));
    ...
    public boolean exist(int codigo){
        Cursor cursor= db.select("select * from TIPOCLIENTE WHERE CODIGO = ?",new String[]{String.valueOf(codigo)});
    ...
    public long getLastUpdate(){
        ...
        Cursor c = db.select("select MAX(DATETIME("+C_ACTUALIZACION+")) as MAX FROM TIPOCLIENTE",null);
        if(c!=null){
            if(c.moveToFirst()){
                do{
    ...
    /**
     * Cargas los datos desde un ArrayList TipoCliente hacia la bdd local
     * @param tipos
     */
    public void load(ArrayList<TipoCliente> tipos){
        int partes=(int)tipos.size()/500;
        int total=tipos.size();
        Log.i("N tran 500:", ":" + partes);
        for (int i = 1; i <= partes+1 ; i++) {
            ...
            try{
                ...
                while((j<(limite))){
                    Log.i("Loading TipoCliente", ":"+j);
                    TipoCliente tipo = tipos.get(j++);
